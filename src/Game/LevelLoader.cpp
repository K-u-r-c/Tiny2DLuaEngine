...
void LevelLoader::LoadLevel(sol::state& lua, const std::unique_ptr<Registry>& registry, const std::unique_ptr<AssetManager>& assetManager,
                            SDL_Renderer* renderer, std::string gamePath, int levelNumber) {
    sol::load_result script = lua.load_file(gamePath + "/assets/scripts/Level" + std::to_string(levelNumber) + ".lua");
    // ? This checks the syntax of our script but does not execute the script
    if (!script.valid()) {
        sol::error err = script;
        std::string error = err.what();
        std::string errorMessage = "Error loading the lua script: " + error;
        perror(errorMessage.c_str());
        return;
    }
    lua.script_file(gamePath + "/assets/scripts/Level" + std::to_string(levelNumber) + ".lua");
    ...
    while (true) {
        ...
        if (assetType == "texture") {
            assetManager->AddTexture(renderer, assetId, assetFilePath);
        }

        if (assetType == "font") {
            assetManager->AddFont(assetId, assetFilePath, asset["font_size"]);
        }
    ...
    std::fstream mapFile;
    mapFile.open(gamePath + mapFilePath);
    ...
    while (true) {
        ...
        if (hasComponents != sol::nullopt) {
            ...
            if (rigidbody != sol::nullopt) {
                newEntity.AddComponent<RigidbodyComponent>(glm::vec2(entity["components"]["rigidbody"]["velocity"]["x"].get_or(0.0),
            ...
            if (sprite != sol::nullopt) {
                newEntity.AddComponent<SpriteComponent>(
                    entity["components"]["sprite"]["texture_asset_id"], entity["components"]["sprite"]["width"],
                    entity["components"]["sprite"]["height"], entity["components"]["sprite"]["z_index"].get_or(1),
                    entity["components"]["sprite"]["fixed"].get_or(false), entity["components"]["sprite"]["src_rect_x"].get_or(0),
            ...
            if (collider != sol::nullopt) {
                newEntity.AddComponent<BoxColliderComponent>(entity["components"]["boxcollider"]["width"],
            ...
            if (health != sol::nullopt) {
                newEntity.AddComponent<HealthComponent>(static_cast<int>(entity["components"]["health"]["health_percentage"].get_or(100)));
            }
            ...
            if (projectileEmitter != sol::nullopt) {
                newEntity.AddComponent<ProjectileEmitterComponent>(
                    ...
                    static_cast<double>(entity["components"]["projectile_emitter"]["repeat_frequency"].get_or(1.0)) * 1000,
                    ...
                    entity["components"]["projectile_emitter"]["is_repeat"].get_or(false));
            }
